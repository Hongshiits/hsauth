...
file_in=open('config.json')
config_json_str=file_in.read()
file_in.close()
config_digested=json.loads(config_json_str)
...
signature_key=config_digested["server_signature"]
...
def send_email(email_in:str,host_in=None,add_type=None):
    otp=generate_token(email_in,config_digested["email_out_of_time_seconds"],config_digested["token_hash"])
    smtp_obj=smtplib.SMTP(config_digested['email_smtp_host'],config_digested["email_smtp_host_port"])
    smtp_obj.ehlo()
    smtp_obj.starttls()
    smtp_obj.ehlo()
    smtp_obj.login(config_digested['email_smtp_account'],config_digested['email_smtp_password'])
    if add_type is None:
        message=MIMEText(mail_template_general['register']%(host_in,otp,email_in),'html','utf-8')
    else:
...
db_obj=pymysql.connect(host=config_digested["mysql_host"],port=config_digested["mysql_port"],user=config_digested["mysql_user"],passwd=config_digested["mysql_passwd"],database=config_digested["mysql_database"])
...
register_server_num=config_digested["register_server_num"]
register_db_num=config_digested["register_db_num"]
...
for mail_type_in,mail_template_path_in in config_digested['email_template'].items():
    print("email type:"+mail_type_in,"  email template path:"+mail_template_path_in)
    file_in=open(os.path.join(BASE_DIR,str(mail_template_path_in)),encoding='utf-8')
    mail_template_general[mail_type_in]=file_in.read()
    file_in.close()
    pass
        ...
        try:
            ...
            if db_data is not None and db_data[2]==req['password']: # type: ignore
                ...
                if req['is_long_term_login']=='True':
                    res['data']['time']=config_digested["login_long_term_seconds"]
                    res['data']['token'] =generate_token(str(db_data[0]), config_digested["login_long_term_seconds"],config_digested['token_hash'],db_data[2])# type: ignore
                    pass
                else:
                    res['data']['time']=config_digested["login_short_term_seconds"]
                    res['data']['token'] =generate_token(str(db_data[0]),config_digested["login_short_term_seconds"],config_digested['token_hash'],db_data[2])# type: ignore
                    pass
                pass
            else: 
